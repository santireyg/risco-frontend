...
import { formatCUIT, extractDateString } from "../../utils/formatting";

const formatDateDDMMYYYY = (value: any): string => {
  // Extraer el string de fecha del objeto MongoDB o valor directo
...
export default function CompanyHeader({
  ...
  return (
    <Card className="border-none bg-transparent" radius="none" shadow="none">
      <CardBody className="px-0 pb-8 pt-0">
        <div className="flex flex-col gap-6 border-gray-200">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-col gap-3">
              <div className="flex items-center  sm:gap-3">
                ...
                <h1 className="text-3xl font leading-tight text-slate-900">{companyInfo.company_name}</h1>
              </div>
          ...
          {isBCRADataMissing && (
            <div className="flex items-start gap-3 rounded-lg border border-warning bg-transparent p-4">
              <ExclamationTriangleIcon className="h-5 w-5 shrink-0 text-warning" />
              <div className="flex flex-col gap-1">
                <p className="text-sm font-medium text-warning">No se encontró información del BCRA</p>
                <p className="text-sm text-slate-600">
                  No se pudo obtener información del BCRA para el CUIT ingresado. Esto puede indicar que el CUIT es incorrecto o
                  no está registrado en el sistema del BCRA.
                  {docfileId && (
                    <>
                      {" "}
                      <Link className="text-primary hover:underline" href={`/document/${docfileId}`}>
                        Puede revisar y editar los datos del documento aquí.
                      </Link>
                    </>
                  )}
                </p>
