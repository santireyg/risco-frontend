...
export default function ResumenEjecutivo({
  ...
  const kpis = useMemo(() => {
    ...
    return allKPIs.filter(
      (kpi) =>
        ...
        kpi.name !== "% CT sobre Activo Total",
    );
  }, [reporteData.indicators]);

  // Find Capital de Trabajo indicators
  const capitalTrabajoIndicator = reporteData.indicators.find(
    (i) => i.code === "capital_trabajo",
  );
  const ctSobreActivoIndicator = reporteData.indicators.find(
    (i) => i.code === "ct_sobre_activo",
  );
  const rotacionCTIndicator = reporteData.indicators.find(
    (i) => i.code === "rotacion_ct",
  );
  ...
  const capitalTrabajoData = {
    ...
    variation: capitalTrabajoIndicator
      ? calculateVariation(
          capitalTrabajoIndicator.value_current,
          capitalTrabajoIndicator.value_previous,
        ) / 100 // Divide by 100 because formatPercentage expects a decimal (0.25 for 25%)
      : 0,
  ...
  const statementDate = formatDate(
    reporteData.statement_data.statement_date.$date,
  );
