...
export default function ChequesRechazados({ data }: ChequesRechazadosProps) {
  ...
  const todosCheques = data.results.causales.flatMap((causal) => causal.entidades.flatMap((entidad) => entidad.detalle));

  // Cantidad total
  const cantidadTotal = todosCheques.length;

  // Cheques impagos (sin fechaPago o fecha futura)
  const chequesImpagos = todosCheques.filter((cheque) => !cheque.fechaPago || new Date(cheque.fechaPago) > new Date());
  const cantidadImpagos = chequesImpagos.length;

  // Monto total
  const montoTotal = todosCheques.reduce((sum, cheque) => sum + cheque.monto, 0);
    ...
    <Card shadow="none" radius="sm" className="border border-slate-200 shadow-sm">
      <CardBody className="p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-xl font-bold text-gray-900">Cheques Rechazados</h3>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="p-4 bg-gray-50 rounded-lg">
            ...
            <div className="text-2xl font-bold text-gray-900">{cantidadTotal}</div>
          </div>

          <div className={`p-4 rounded-lg ${cantidadImpagos > 0 ? "bg-danger/10" : "bg-gray-50"}`}>
            <div className="flex items-center gap-2 mb-2">
              <Clock className="w-4 h-4 text-gray-600" />
              <div className="text-xs text-gray-600">Pendientes Pago</div>
            </div>
            <div className={`text-2xl font-bold ${cantidadImpagos > 0 ? "text-danger" : "text-gray-900"}`}>{cantidadImpagos}</div>
          </div>
          ...
          <div className="p-4 bg-gray-50 rounded-lg">
            ...
            <div className="text-lg font-bold text-gray-900">{formatCurrency(montoTotal)}</div>
          </div>

          <div className={`p-4 rounded-lg ${montoImpagos > 0 ? "bg-danger/10" : "bg-gray-50"}`}>
            <div className="flex items-center gap-2 mb-2">
              <AlertTriangle className="w-4 h-4 text-gray-600" />
              <div className="text-xs text-gray-600">Monto Impagos</div>
            </div>
            <div className={`text-lg font-bold ${montoImpagos > 0 ? "text-danger" : "text-gray-900"}`}>
          ...
          <div className="mt-4 p-3 bg-danger/10 border border-danger/20 rounded-lg">
            <div className="flex items-start gap-2">
              ...
              <p className="text-xs text-danger">
                <strong>Alerta:</strong> Se detectaron cheques pendientes de pago. Esto representa un riesgo crediticio
                significativo.
              </p>
            </div>
          </div>
