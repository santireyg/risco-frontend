...
import { TrendingUp, TrendingDown, Minus, Activity, Droplets, Wallet, PieChart, TrendingUpDown } from "lucide-react";
    ...
    <Card shadow="none" radius="sm" className="border border-slate-200 shadow-sm">
      <CardBody className="p-6">
        ...
        <div className="space-y-3">
          {kpis.map((kpi) => {
            const variacion = calcularVariacion(kpi.value, kpi.comparison.value);
            const isPercentageKPI = kpi.name.includes("Endeudamiento") || kpi.name.includes("Margen");
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isExpanded ? "border-primary/30 shadow-md" : "hover:border-primary/50 hover:shadow-md"
                )}>
                <button
                  type="button"
                  className={cn(
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-semibold text-gray-900 mb-1">{kpi.name}</div>
                    <div className="flex items-center gap-3">
                      <span className="text-lg font-bold text-gray-900">
                        {isPercentageKPI ? formatPercentage(kpi.value) : formatNumber(kpi.value)}
                      </span>
                      <div className="flex items-center gap-1.5">
                        <span className={cn("w-2 h-2 rounded-full", getDotColor(kpi.status))} />
                        <span className={cn("text-xs font-medium", getKPIStatusColor(kpi.status))}>
                  ...
                  <div className="flex-shrink-0 flex items-center gap-2">{getTrendIcon(kpi.comparison.trend)}</div>
                </button>
                {isExpanded && (
                  <div className="px-4 pb-4 text-xs text-gray-600">
                    <div className="pt-3">
                      <div className="flex items-baseline gap-2">
                        <span className="text-base font-bold text-primary">
                          {isPercentageKPI ? formatPercentage(kpi.value, 0) : formatNumber(kpi.value, 2)}
                        </span>
                        <span className={cn("text-xs font-semibold", variacion >= 0 ? "text-success" : "text-danger")}>
                          {variacion >= 0 ? "+" : ""}
                      ...
                      <div className="mt-1 text-[11px] text-gray-500">
                        Anterior:{" "}
                        {isPercentageKPI ? formatPercentage(kpi.comparison.value, 0) : formatNumber(kpi.comparison.value, 2)}
                      </div>
                    </div>

                    <div className="mt-3">
                      <div className="text-[11px] font-semibold text-gray-700 mb-1.5 uppercase tracking-wide">Criterios</div>
                    ...
                    <div className="mt-3">
                      <div className="text-[11px] font-semibold text-gray-700 mb-1 uppercase tracking-wide">Descripción</div>
                    ...
                    <div className="mt-3">
                      ...
                      <div className="font-mono bg-gray-50 p-2 rounded text-gray-700">{kpi.formula}</div>
                    </div>
                  </div>
                )}
              </div>
