...
export default function EstadoDeudor({ data }: EstadoDeudorProps) {
  ...
  const getSituacionChip = (sit: number) => {
    if (sit === 0) return { chip: "Situación 0", description: "Sin deuda", color: "default" as const };
    if (sit === 1) return { chip: "Situación 1", description: "En situación normal", color: "success" as const };
    if (sit === 2) return { chip: "Situación 2", description: "Con seguimiento especial", color: "warning" as const };
    if (sit === 3) return { chip: "Situación 3", description: "Con problemas", color: "warning" as const };
    if (sit === 4) return { chip: "Situación 4", description: "Con alto riesgo de insolvencia", color: "danger" as const };
    if (sit === 5) return { chip: "Situación 5", description: "Irrecuperable", color: "danger" as const };
    if (sit === 6) return { chip: "Situación 6", description: "Irrecuperable por disposición técnica", color: "danger" as const };

    return { chip: `Situación ${sit}`, description: "Crítico", color: "danger" as const };
  };
      ...
      <CardBody className="p-6">
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div className="flex flex-col items-end">
            ...
            <div className="text-xs text-gray-600 mt-1">{situacion.description}</div>
          </div>
        </div>
        ...
        <div>
          ...
          <ResponsiveContainer height={120} width="100%">
            <LineChart data={chartData}>
              <XAxis axisLine={false} dataKey="periodoFormat" tick={{ fontSize: 10 }} tickLine={false} />
              <YAxis
                axisLine={false}
                orientation="right"
                tick={{ fontSize: 10 }}
