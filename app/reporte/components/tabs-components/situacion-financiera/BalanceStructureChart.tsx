...
export default function BalanceStructureChart({ data }: BalanceStructureChartProps) {
  // Sort data 2024 first
  const sortedData = [...data].sort((a, b) => Number(b.year) - Number(a.year));

  return (
    <Card className="border border-slate-200 shadow-sm w-full" radius="sm" shadow="none">
      ...
      <CardBody className="overflow-visible pb-4 flex flex-col gap-4">
        <div className="w-full h-[350px]">
          <ResponsiveContainer height="100%" width="100%">
            <BarChart
              ...
              <XAxis axisLine={false} dataKey="year" tick={{ fill: "#6B7280" }} tickLine={false} />
              <YAxis
                axisLine={false}
              ...
              {/* Activo Stack */}
              <Bar dataKey="activoNoCorriente" fill={COLORS.activoNoCorriente} radius={[0, 0, 0, 0]} stackId="activo">
                <LabelList content={<RenderLabel dataKey="activoNoCorriente" />} />
              </Bar>
              <Bar dataKey="activoCorriente" fill={COLORS.activoCorriente} radius={[4, 4, 0, 0]} stackId="activo">
                <LabelList content={<RenderLabel dataKey="activoCorriente" />} />
              </Bar>

              {/* Pasivo + PN Stack */}
              <Bar dataKey="patrimonioNeto" fill={COLORS.patrimonioNeto} radius={[0, 0, 0, 0]} stackId="pasivo">
                <LabelList content={<RenderLabel dataKey="patrimonioNeto" />} />
              </Bar>
              <Bar dataKey="pasivoNoCorriente" fill={COLORS.pasivoNoCorriente} radius={[0, 0, 0, 0]} stackId="pasivo">
                <LabelList content={<RenderLabel dataKey="pasivoNoCorriente" />} />
              </Bar>
              <Bar dataKey="pasivoCorriente" fill={COLORS.pasivoCorriente} radius={[4, 4, 0, 0]} stackId="pasivo">
        ...
        <div className="mt-4 flex flex-wrap justify-center gap-x-4 gap-y-2 px-2">
          <div className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full" style={{ background: COLORS.activoCorriente }} />
            <span className="text-[10px] text-gray-500 font-medium">
              A<sub className="align-baseline text-[8px]">C</sub> (Activo Cte.)
            </span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full" style={{ background: COLORS.activoNoCorriente }} />
            <span className="text-[10px] text-gray-500 font-medium">
              A<sub className="align-baseline text-[8px]">NC</sub> (Activo No Cte.)
            </span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full" style={{ background: COLORS.pasivoCorriente }} />
            <span className="text-[10px] text-gray-500 font-medium">
              P<sub className="align-baseline text-[8px]">C</sub> (Pasivo Cte.)
            </span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full" style={{ background: COLORS.pasivoNoCorriente }} />
            <span className="text-[10px] text-gray-500 font-medium">
              P<sub className="align-baseline text-[8px]">NC</sub> (Pasivo No Cte.)
            </span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full" style={{ background: COLORS.patrimonioNeto }} />
