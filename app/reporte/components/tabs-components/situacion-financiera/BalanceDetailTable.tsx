...
interface BalanceDetailTableProps {
  items: FinancialDetailItem[];
}

export default function BalanceDetailTable({ items }: BalanceDetailTableProps) {
  ...
  return (
    <div className="w-full relative rounded-lg border border-gray-100 overflow-hidden">
      {/* // <div className=""> */}
      <div
          ...
          <Table
            ...
            <TableBody>
              {items.map((item, index) => {
                ...
                const varPt = item.monto_periodo_anterior !== 0 ? (diff / Math.abs(item.monto_periodo_anterior)) * 100 : 0;

                return (
                  <TableRow key={index}>
                    <TableCell className="max-w-[200px] truncate" title={item.concepto}>
                      {item.concepto}
                    </TableCell>
                    <TableCell className="text-right tabular-nums text-gray-700">
                      {formatMoneyInt(item.monto_periodo_actual)}
                    </TableCell>
                    <TableCell className="text-right tabular-nums text-gray-500">
                      {formatMoneyInt(item.monto_periodo_anterior)}
                    </TableCell>
